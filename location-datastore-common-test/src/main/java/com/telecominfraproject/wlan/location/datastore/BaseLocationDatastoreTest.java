package com.telecominfraproject.wlan.location.datastore;
 ...
 *
 */
public abstract class BaseLocationDatastoreTest {
    @Autowired
    protected LocationDatastore testInterface;

    private static final AtomicInteger customerSequence = new AtomicInteger(2);
    ...
    public void testGetAllForCustomer() {
        ...
        for(int i=0; i<5; i++){
            Location location = new Location();
            location.setLocationType(LocationType.SITE);
            location.setCustomerId(customerId_2);
            location.setName("testName_"+i);
    ...
    public void testChildrenAndAncestors()
    {
       ...
       try {
    	   testInterface.delete(grandpa.getId());
    	   fail("deleted location that has descendants");
       } catch(Exception e) {
    	   //expected it
       }
    ...
    public void testLocationPagination()
    {
       ...
       Location mdl;
       int customerId_1 = customerSequence.incrementAndGet();
       int customerId_2 = customerSequence.incrementAndGet();
