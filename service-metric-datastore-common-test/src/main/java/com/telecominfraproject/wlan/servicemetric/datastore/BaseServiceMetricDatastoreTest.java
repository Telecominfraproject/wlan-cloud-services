...
public abstract class BaseServiceMetricDatastoreTest {
    ...
    public void testCRUD() {
    	ServiceMetric serviceMetric = createServiceMetricObject();

        //create
    	ServiceMetric created = testInterface.create(serviceMetric);
        assertNotNull(created);
        assertTrue(created.getId() > 0);
        assertEquals(serviceMetric.getSampleStr(), created.getSampleStr());
        assertEquals(serviceMetric.getCustomerId(), created.getCustomerId());
        assertNotNull(created.getDetails());
        assertEquals(serviceMetric.getDetails(), created.getDetails());
                
        // update
        created.setSampleStr(created.getSampleStr()+"_updated");
        ServiceMetric updated = testInterface.update(created);
        assertNotNull(updated);
        assertEquals(created.getSampleStr(), updated.getSampleStr());
        
        //UPDATE test - fail because of concurrent modification exception
        try{
        	ServiceMetric modelConcurrentUpdate = created.clone();
        ...
        //retrieve
        ServiceMetric retrieved = testInterface.get(created.getId());
    ...
    public void testGetAllInSet() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            serviceMetric.setCustomerId(i);

            ServiceMetric ret = testInterface.create(serviceMetric);
        ...
        List<ServiceMetric> serviceMetricsRetrievedByIdSet = testInterface.get(testSetIds);
    ...
    public void testServiceMetricPagination()
    {
       ...
       PaginationResponse<ServiceMetric> page1 = testInterface.getForCustomer(customerId_1, sortBy, context);
       PaginationResponse<ServiceMetric> page2 = testInterface.getForCustomer(customerId_1, sortBy, page1.getContext());
       PaginationResponse<ServiceMetric> page3 = testInterface.getForCustomer(customerId_1, sortBy, page2.getContext());
       PaginationResponse<ServiceMetric> page4 = testInterface.getForCustomer(customerId_1, sortBy, page3.getContext());
       PaginationResponse<ServiceMetric> page5 = testInterface.getForCustomer(customerId_1, sortBy, page4.getContext());
       PaginationResponse<ServiceMetric> page6 = testInterface.getForCustomer(customerId_1, sortBy, page5.getContext());
       PaginationResponse<ServiceMetric> page7 = testInterface.getForCustomer(customerId_1, sortBy, page6.getContext());
...
//       System.out.println("================================");
       ...
       PaginationResponse<ServiceMetric> page1EmptySort = testInterface.getForCustomer(customerId_1, Collections.emptyList(), context);
       ...
       PaginationResponse<ServiceMetric> page1NullSort = testInterface.getForCustomer(customerId_1, null, context);
       ...
       PaginationResponse<ServiceMetric> page1SingleSortDesc = testInterface.getForCustomer(customerId_1, Collections.singletonList(new ColumnAndSort("sampleStr", SortOrder.desc)), context);
    ...
    private ServiceMetric createServiceMetricObject() {
    	ServiceMetric result = new ServiceMetric();
        long nextId = testSequence.getAndIncrement();
        result.setCustomerId((int) nextId);
        result.setSampleStr("test-" + nextId); 
        ServiceMetricDetails details = new ServiceMetricDetails();
