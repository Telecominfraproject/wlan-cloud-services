...
public class ApElementConfiguration extends CommonElementConfiguration {
     ...
     * the associated AP Profile or through tweaking the global defaults.
     * 
     */
    ...
    public static ApElementConfiguration createWithDefaults(String elementVersion, ApModel model) {
        ...
        // TODO: MikeH Do we have outdoor AP, if so, need U/L Band channels for
        // defaults?
    ...
    public boolean hasUnsupportedValue() {
        ...
        /*
         * These are the "advanced" radio configuration. They can be redefined
         * via the associated AP Profile or through tweaking the global
         * defaults.
         * 
         */
    ...
    public boolean needsToBeUpdatedOnDevice(Object previousVersion) {
        if (previousVersion instanceof ApElementConfiguration) {
            ...
            if (radioMapsNeedUpdate(casted)) return true;
    ...
    private boolean radioMapsNeedUpdate(ApElementConfiguration casted) {
        for (RadioType radioType : casted.getAdvancedRadioMap().keySet()) {
            if (!this.getAdvancedRadioMap().containsKey(radioType))
                return true;
            if (this.getAdvancedRadioMap().get(radioType)
                    .needsToBeUpdatedOnDevice(casted.getAdvancedRadioMap().get(radioType)))
                return true;
        }
        
        for (RadioType radioType : casted.getRadioMap().keySet()) {
            if (!this.getRadioMap().containsKey(radioType))
                return true;
            if (this.getRadioMap().get(radioType)
                    .needsToBeUpdatedOnDevice(casted.getRadioMap().get(radioType)))
                return true;
        }
