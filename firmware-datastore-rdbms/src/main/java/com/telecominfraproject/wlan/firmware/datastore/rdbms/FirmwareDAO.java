...
public class FirmwareDAO extends BaseJdbcDao {
    ...
    private static final String[] ALL_COLUMNS_LIST = {        
        ...
        //TODO: add colums from properties Firmware in here
        "customerId",
        "sampleStr",
        "details",
        //make sure the order of properties matches this list and list in FirmwareRowMapper and list in create/update methods
    ...
    private static final String TABLE_NAME = "firmware";
    ...
    private static final RowMapper<Firmware> firmwareRowMapper = new FirmwareRowMapper();
    ...
    public Firmware create(final Firmware firmware) {
            ...
            jdbcTemplate.update(
                new PreparedStatementCreator() {
                    public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
                        ...
                        //TODO: add remaining properties from Firmware here 
                        ps.setInt(colIdx++, firmware.getCustomerId());
                        ps.setString(colIdx++, firmware.getSampleStr());
                      	ps.setBytes(colIdx++, (firmware.getDetails()!=null)?firmware.getDetails().toZippedBytes():null);
        ...
        firmware.setId(((Long)keyHolder.getKeys().get(COL_ID)));
        firmware.setCreatedTimestamp(ts);
        firmware.setLastModifiedTimestamp(ts);


        LOG.debug("Stored Firmware {}", firmware);
    ...
    public Firmware get(long firmwareId) {
        ...
        try{
            Firmware firmware = this.jdbcTemplate.queryForObject(
                    SQL_GET_BY_ID,
                    firmwareRowMapper, firmwareId);
    ...
    public Firmware getOrNull(long firmwareId) {
        ...
        try{
            ...
            LOG.debug("Could not find Firmware for id {}", firmwareId);
            return null;
        }
    }
    ...
    public Firmware update(Firmware firmware) {
        ...
        long incomingLastModifiedTs = firmware.getLastModifiedTimestamp();
        
        int updateCount = this.jdbcTemplate.update(SQL_UPDATE, new Object[]{ 
                //firmware.getId(), - not updating this one

                //TODO: add remaining properties from Firmware here
        		firmware.getCustomerId(),
                firmware.getSampleStr(),
                (firmware.getDetails()!=null)?firmware.getDetails().toZippedBytes():null ,
                                
                //firmware.getCreatedTimestamp(), - not updating this one
                newLastModifiedTs,
        ...
        if(updateCount==0){
            ...
            try{
                ...
                LOG.debug("Cannot find Firmware for {}", firmware.getId());
                throw new DsEntityNotFoundException("Firmware not found " + firmware.getId());
            }
        }

        //make a copy so that we don't accidentally update caller's version by reference
        Firmware firmwareCopy = firmware.clone();
        firmwareCopy.setLastModifiedTimestamp(newLastModifiedTs);
    ...
    public Firmware delete(long firmwareId) {
        Firmware ret = get(firmwareId);
        
        this.jdbcTemplate.update(SQL_DELETE, firmwareId);
    ...
    public List<Firmware> getAllForCustomer(int customerId) {
        LOG.debug("Looking up Firmwares for customer {}", customerId);

        List<Firmware> ret = this.jdbcTemplate.query(SQL_GET_BY_CUSTOMER_ID,
                firmwareRowMapper, customerId);

        if (ret == null) {
            LOG.debug("Cannot find Firmwares for customer {}", customerId);
        } else {
            LOG.debug("Found Firmwares for customer {} : {}", customerId, ret);
        }
    ...
    public List<Firmware> get(Set<Long> firmwareIdSet) {
        LOG.debug("calling get({})", firmwareIdSet);
        ...
        set.append("(");
        for(int i =0; i< firmwareIdSet.size(); i++) {
        		set.append("?,");
        }
        //remove last comma
        ...
        List<Firmware> results = this.jdbcTemplate.query(query, firmwareIdSet.toArray(), firmwareRowMapper);

        LOG.debug("get({}) returns {} record(s)", firmwareIdSet, (null == results) ? 0 : results.size());
        return results;
    }
	...
	public PaginationResponse<Firmware> getForCustomer(int customerId, List<ColumnAndSort> sortBy,
         ...
         */
        List<Firmware> pageItems = this.jdbcTemplate.query(query, queryArgs.toArray(),
                firmwareRowMapper);

        if (pageItems == null) {
            LOG.debug("Cannot find Firmwares for customer {} with last returned page number {}",
                    customerId, context.getLastReturnedPageNumber());
        } else {
            LOG.debug("Found {} Firmwares for customer {} with last returned page number {}",
