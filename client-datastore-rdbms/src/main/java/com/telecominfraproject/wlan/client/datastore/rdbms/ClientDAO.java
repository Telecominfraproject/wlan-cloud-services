...
public class ClientDAO extends BaseJdbcDao {
    ...
    public Client create(final Client client) {
            ...
            jdbcTemplate.update(
                new PreparedStatementCreator() {
                    public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
                        ...
                        ps.setString(colIdx++, client.getSampleStr());
        ...
        client.setId(((Long)keyHolder.getKeys().get(COL_ID)));
    ...
    public Client getOrNull(long clientId) {
        ...
        try{
            ...
            LOG.debug("Could not find Client for id {}", clientId);
            return null;
        }
    }
    ...
    public Client update(Client client) {
        ...
        int updateCount = this.jdbcTemplate.update(SQL_UPDATE, new Object[]{ 
                //client.getId(), - not updating this one
        ...
        if(updateCount==0){
            ...
            try{
                ...
                LOG.debug("Cannot find Client for {}", client.getId());
                throw new DsEntityNotFoundException("Client not found " + client.getId());
            }
        }
    ...
    public Client delete(long clientId) {
        Client ret = get(clientId);
        
        this.jdbcTemplate.update(SQL_DELETE, clientId);
    ...
    public List<Client> get(Set<Long> clientIdSet) {
        LOG.debug("calling get({})", clientIdSet);
        ...
        set.append("(");
        for(int i =0; i< clientIdSet.size(); i++) {
        		set.append("?,");
        }
        //remove last comma
        ...
        List<Client> results = this.jdbcTemplate.query(query, clientIdSet.toArray(), clientRowMapper);

        LOG.debug("get({}) returns {} record(s)", clientIdSet, (null == results) ? 0 : results.size());
        return results;
    }
